...
For pvAccess PVs, the C<alarm.message> of the NTScalar may be populated with 
strings in the INFO fields AMSG_HIHI, AMSG_HIGH, AMSG_LOW and AMSG_LOLO, 
e.g. C<info(BASE:AMSG_HIHI, "Alarm message for HIHI")>. These fields set the 
AMSG and may be further overridden by submodules that set the AMSG directly. 
If not set, the C<alarm.message> is set by the module managing the pvAccess 
PV.

| Info Field | Summary | Type | DCT | Default | Read | Write | CA PP |
|------------|---------|------|-----|---------|------|-------|-------|
| BASE:AMSG_HIHI | Hihi Alarm Message | STRING[40] | No | | No | No | No |
| BASE:AMSG_HIGH | High Alarm Message | STRING[40] | No | | No | No | No |
| BASE:AMSG_LOW | Low Alarm Message | STRING[40] | No | | No | No | No |
| BASE:AMSG_LOLO | Lolo Alarm Message | STRING[40] | No | | No | No | No |
...
=cut
	...
	field(ALARM_MSG_HIHI,DBF_NOACCESS) {
		prompt("Hihi Alarm Message")
		special(SPC_NOMOD)
		interest(4)
		extra("char                *alarm_msg_hihi")
	}
    field(ALARM_MSG_HIGH,DBF_NOACCESS) {
		prompt("High Alarm Message")
		special(SPC_NOMOD)
		interest(4)
		extra("char                *alarm_msg_high")
	}
    field(ALARM_MSG_LOLO,DBF_NOACCESS) {
		prompt("Lolo Alarm Message")
		special(SPC_NOMOD)
		interest(4)
		extra("char                *alarm_msg_lolo")
	}
    field(ALARM_MSG_LOW,DBF_NOACCESS) {
		prompt("Low Alarm Message")
		special(SPC_NOMOD)
		interest(4)
		extra("char                *alarm_msg_low")
	}
